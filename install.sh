...
if [ $SUDO -eq 1 ]; then
    if [ "$OS" = "debian" ]; then
        ...
        #core
        $PACMAN base-devel bash busybox bzip2 coreutils e2fsprogs fzf gnupg gnutls gzip hdparm htop iotop iperf less lm_sensors lsb-release lshw lsof make nano neovim openssh openssl procps-ng psmisc readline rsync sudo tar time tmux tree udiskie vi zip zsh pass apparmor firejail man
        #networking
...
if [ $SXMO -eq 1 ]; then
    CONFIGNAMES=("aerc" "sxiv" "tm" "ncmpcpp.config" "lf" "sxmo" "tuir" "tut" "newsboat" "user-dirs.dirs")
elif [ -z "$NO_DESKTOP" ]; then
    CONFIGNAMES=("aerc" "nvim" "ranger" "sxiv" "ipython" "tm" "ncmpcpp.config" "lf" "zathura" "tuir" "tut" "newsboat" "dunst" "sway" "mako" "i3status" "foot" "rofi" "user-dirs.dirs" "waybar" "hypr")
else
    CONFIGNAMES=("nvim" "ranger" "ipython" "tm" "lf")
fi
