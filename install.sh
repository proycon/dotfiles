...
if [ $SUDO -eq 1 ]; then
    if [[ "$OS" == "debian" ]]; then
        ...
        if [ -z "$NO_DESKTOP" ]; then
            sudo -E apt-get $APT_YES install rxvt-unicode compton rofi dmenu i3-wm i3lock openbox newsboat mpv mplayer fcitx firefox pavucontrol ncmpcpp zathura zathura-pdf-poppler gimp inkscape pamixer dunst
