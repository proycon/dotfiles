...
if [ $SUDO -eq 1 ]; then
    if [[ "$OS" == "debian" ]]; then
        ...
        sudo apk update
        sudo apk upgrade
        sudo apk add openssh vim zsh bash tmux htop bat feh newsboat weechat zathura-pdf-mupdf git tig mpv lf python3 fzf tuir espeak sxiv ncdu mpc make gcc libc-dev linux-headers libx11-dev libxft-dev libxcb-dev libxtst-dev freetype-dev libxinerama-dev wqy-zenhei || exit 2
    else
...
if [ $SXMO -eq 1 ]; then
    ...
    CONFIGNAMES=("kitty" "alacritty" "openbox" "nvim" "ranger" "polybar" "sxiv" "i3" "ipython" "tm" "ncmpcpp.config" "vifm" "lf" "broot" "zathura" "tuir" "newsboat" "dunst")
else
