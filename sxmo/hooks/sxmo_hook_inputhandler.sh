#!/bin/sh
# configversion: 2d8f0a7f254b3c68ba16440a76879cac
...
case "$ACTION" in
	"powerbutton_one")
		if echo "$WMCLASS" | grep -i "megapixels"; then
		...
		elif echo "$WMCLASS"  | grep -i "gst-launch"; then
			...
			grim "$FILENAME"
			mpv --really-quiet /usr/share/sounds/freedesktop/stereo/message.oga &
			if zbarimg "$FILENAME" > /tmp/qr; then
				cat /tmp/qr | wl-copy
				QRCODE=$(cat /tmp/qr)
				notify-send --urgency=low "QR Code found" "$QRCODE"
			fi
