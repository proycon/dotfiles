...
MARKUP_LINENO='<span fgcolor="black">\1<\/span>'
MARKUP_PRIORITY_A='<b><span fgcolor="yellow">\1<\/span><\/b>'
MARKUP_PRIORITY_B='<b><span fgcolor="orange">\1<\/span><\/b>'
MARKUP_PRIORITY_C='<b><span fgcolor="darkyellow">\1<\/span><\/b>'
MARKUP_PROJECT='<span fgcolor="darkcyan"><b>\1<\/b><\/span>'
MARKUP_CONTEXT='<span fgcolor="darkmagenta"><b>\1<\/b><\/span>'
MARKUP_TAG='<span fgcolor="darkgreen"><b>\1<\/b><\/span>'
MARKUP_DUE='<span fgcolor="red"><b>\1<\/b><\/span>'
MARKUP_URL='<span fgcolor="blue"><b>\1<\/b><\/span>'
...
runrofi () {
    $ROFI_BIN -matching glob -tokenize -i -no-levenshtein-sort "$@"
}
...
highlight() {
    ...
    WORD_REGEX="[[:alnum:]]+"
    echo "${line}" | sed -r "
        s/^([[:digit:]]+)/${MARKUP_LINENO}/g;
        s/(\(A\))/${MARKUP_PRIORITY_A}/g;
        s/(\(B\))/${MARKUP_PRIORITY_B}/g;
        s/(\(C\))/${MARKUP_PRIORITY_C}/g;
        s/(\+${WORD_REGEX})/${MARKUP_PROJECT}/g;
        s/(\@${WORD_REGEX})/${MARKUP_CONTEXT}/g;
        s/(\#${WORD_REGEX})/${MARKUP_TAG}/g;
        s/(https?:\/\/.*)/${MARKUP_URL}/g;
...
unhighlight() {
    ...
    UNMARKUP_URL="${MARKUP_URL/\\1/${REGEX}}"
