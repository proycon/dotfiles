...
menu () {
    ...
    if [[ "$choice " == "sysadmin"* ]]; then
        whiptail --menu "Menu" 25 80 15 btop "Interactive process viewer" dbus-monitor "DBUS Monitor" htop "Interactive process viewer"  iostat "I/O Statistics" iotop "I/O monitor (root)" lshw "List hardware" lsmod "List kernel modules" lsof "List open files" lspci "List PCI devices" lsusb "List USB devices" netcat "Read/write network data" netstat "Print network connections" pkgtop "Package manager" top "Interactive process viewer" tspin "Tailspin: highlighting for logs" udevadm "Device administration" vmstat "Report virtual memory statistics" 2> ~/.menuchoice
        eval $(cat ~/.menuchoice)
